...
for nPuVtx in nPuVtxset :
    ...
    for alpha in alphaset :
        ...
        for et in enlist :
            ...
            if opt.phi!=0.5 : outDir='%s/git%s/version%d/%s/200um/phi_%3.3fpi/eta%s_et%s_pu%s'%(opt.out,opt.gittag,opt.version,opt.datatype,opt.phi,eta,et,nPuVtx)
            ...
            if (nPuVtx==0) :
                if (opt.nRuns==0) :
                    scriptFile.write('%s/bin/egammaResolution -c scripts/DefaultConfig.cfg -n %s -i root://eoscms//eos/cms%s/ --digifilePath=root://eoscms//eos/cms%s/ -s HGcal_%s.root -r Digi_%s.root -o %s.root --redoStep=%s | tee %s\n'%(os.getcwd(),opt.nevts,eosDirIn,eosDir,outTag,outTag,outDir,redofit,outlog))
                else:
                    scriptFile.write('%s/bin/egammaResolution -c scripts/DefaultConfig.cfg -n %s --nRuns=%s -i root://eoscms//eos/cms%s/ --digifilePath=root://eoscms//eos/cms%s/ -s HGcal_%s -r Digi_%s -o %s.root --redoStep=%s | tee %s\n'%(os.getcwd(),opt.nevts,opt.nRuns,eosDirIn,eosDir,outTag,outTag,outDir,redofit,outlog))
            else:
                if (opt.nRuns==0) :
