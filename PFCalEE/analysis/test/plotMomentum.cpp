...
int main(int argc, char ** argv) {
  ...
  TFile *fout = TFile::Open("PLOTS/output_hepmc.root","RECREATE");
  fout->cd();
  ...
  TH1F *hvtx_t = new TH1F("hvtx_t",";t (ns);vertices",1000,-1000,1000);
  ...
  for (unsigned i(0);i<nFiles;++i){
	...
	hvtx_z->Fill((*q)->position().z());
	hvtx_t->Fill((*q)->position().t());
