...
do
	...
	do
	    ...
	    do
		echo " - Processing point alpha=$alpha, et=$et : " >> check.log
		eos ls /store/user/amagnan/HGCalEEGeant4/$GITVERSION/$p/HGcal_version${v}_model2_BOFF_et${et}_alpha${alpha}.root > /dev/null
		if (( "$?" != 0 )); then
		    echo " --- Sim File does not exist!" >> check.log
		else 
		    eos ls /store/user/amagnan/HGCalEEGeant4/$GITVERSION/$p/Digi_version${v}_model2_BOFF_et${et}_alpha${alpha}.root > /dev/null
		    if (( "$?" != 0 )); then
			echo " --- Rec File does not exist!" >> check.log
		    else
			./bin/checkProduction $INPATH/$GITVERSION/$p/ HGcal_version${v}_model2_BOFF_et${et}_alpha${alpha}.root Digi_version${v}_model2_BOFF_et${et}_alpha${alpha}.root >> check.log
		    fi
		fi
	    done
	done
